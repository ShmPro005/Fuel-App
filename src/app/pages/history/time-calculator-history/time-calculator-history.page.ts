...
@Component({
  selector: 'app-time-calculator-history',
  templateUrl: './time-calculator-history.page.html',
  styleUrls: ['./time-calculator-history.page.scss'],
})
export class TimeCalculatorHistoryPage implements OnInit, ViewWillEnter {
  ...
  ngOnInit() {
    this.pageTitle = this.translateService.instant('TIME_CALCULATOR_HISTORY');
    this.loadRecords();
  }
  ...
  async loadRecords() {
    try {
      ...
      if (!this.records || this.records.length === 0) {
        this.presentToast('No time calculator records found.');
      }
    } catch (error) {
  ...
  async shareRecord(record: any) {
    ...
    if (calculationType) {
      ...
      const appDetails: any = {
        en: calculationType === 'TIME_CALCULATOR' ? `⏱️ Time Calculator Made Easy! 🚗💨
    Check out my time calculation details:

    🛣️ Distance: ${record.distance} KM
    🏎️ Speed: ${record.average} KM/h${optionalDataSection}
    --------------------------------------------
    ⏱️ Total Time: ${record.hours}h ${record.minutes}m ${record.seconds}s

    📅 Date: ${new Date(record.date).toLocaleDateString()}

    Easily calculate travel time with our app!
    📲 Download now: ${shareAppUrl}` : calculationType === 'FUEL_COST' && record.totalHours !== undefined ? `⛽ Machinery Fuel Calculation Made Easy! 🚜💨
    ...
    📲 Download now: ${shareAppUrl}`,
        ...
        hi: calculationType === 'TIME_CALCULATOR' ? `⏱️ समय कैलकुलेटर आसान! 🚗💨
    मेरे समय कैलकुलेशन का विवरण देखें:

    🛣️ दूरी: ${record.distance} KM
    🏎️ गति: ${record.average} KM/h${optionalDataSection}
    ----------------------------------------
    ⏱️ कुल समय: ${record.hours}घंटा ${record.minutes}मिनट ${record.seconds}सेकंड

     दिनांक: ${new Date(record.date).toLocaleDateString()}

    हमारे ऐप से यात्रा समय की गणना करें!
    📲 अभी डाउनलोड करें: ${shareAppUrl}` : calculationType === 'FUEL_COST' && record.totalHours !== undefined ? `⛽ मशीनरी ईंधन गणना आसान! 🚜💨
    ...
    📲 अभी डाउनलोड करें: ${shareAppUrl}`,
        ...
        gu: calculationType === 'TIME_CALCULATOR' ? `⏱️ સમય કેલ્ક્યુલેટર સરળ! 🚗💨
    મારા સમય કેલ્ક્યુલેશનનો વિગતવાર જુઓ:

    🛣️ અંતર: ${record.distance} KM
    🏎️ ઝડપ: ${record.average} KM/h${optionalDataSection}
    ------------------------------------------
    ⏱️ કુલ સમય: ${record.hours}કલાક ${record.minutes}મિનિટ ${record.seconds}સેકન્ડ

    📅 તારીખ: ${new Date(record.date).toLocaleDateString()}

    અમારા એપથી મુસાફરી સમયની ગણતરી કરો!
    📲 હમણાં ડાઉનલોડ કરો: ${shareAppUrl}` : calculationType === 'FUEL_COST' && record.totalHours !== undefined ? `⛽ મશીનરી ઇંધણ ગણતરી સરળ! 🚜💨
