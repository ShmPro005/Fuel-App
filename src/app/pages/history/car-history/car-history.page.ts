...
export class CarHistoryPage implements OnInit, ViewWillEnter {
  ...
  ngOnInit() {
    this.pageTitle = this.translateService.instant('CAR_HISTORY');
    this.loadRecords();
  }
  ...
  async loadRecords() {
    try {
      ...
      if (!this.records || this.records.length === 0) {
        this.presentToast('No car records found.');
      }
    } catch (error) {
