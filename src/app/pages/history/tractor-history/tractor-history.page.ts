...
@Component({
  selector: 'app-tractor-history',
  templateUrl: './tractor-history.page.html',
  styleUrls: ['./tractor-history.page.scss'],
})
export class TractorHistoryPage implements OnInit, ViewWillEnter {
  ...
  pageTitle: string = 'Tractor History';
  ...
  ngOnInit() {
    this.pageTitle = this.translateService.instant('TRACTOR_HISTORY');
    this.loadRecords();
  }
  ...
  async loadRecords() {
    try {
      ...
      if (!this.records || this.records.length === 0) {
        this.presentToast('No tractor records found.');
      }
    } catch (error) {
